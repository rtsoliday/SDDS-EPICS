...
usage: sddswmonitor {<inputfile> | -PVnames=<name>[,<name>]} <outputfile>
    ...
    [-versus=name=<columnName>[,unitsValue=<string>|unitsPV=<string>]
      [,deltaValue=<number>|deltaPV=<string>][,offsetValue=<integer>|offsetPV=<string>]]
...
\begin{itemize}
  ...
  \item {\tt -bandwidthLimitedRMS=<minHz>,<maxHz>[,scaleByOneOverOmegaSquared][,noPV][,minMaxPVName]} ---
          Compute RMS values within a frequency band. With \verb+noPV+, the
          waveform is not stored for this calculation. With \verb+minMaxPVName+,
          any PVs created/connected via the optional \verb+rmsPVPrefix+ input-file
          column use the name pattern \verb+<rmsPVPrefix>:<min>-<max>HzBW+ instead
          of the default \verb+<rmsPVPrefix>:<max>HzBW+.
\end{itemize}
